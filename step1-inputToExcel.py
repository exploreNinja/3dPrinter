...
def process_files():
    ...
    try:
        with open(os.path.join(directory_path, 'x axis.txt'), 'r') as file:
            x_axis_labels = file.read().splitlines()

        # Initialize DataFrame and sort the x_axis_labels if necessary
        x_axis_labels = sorted(x_axis_labels, key=lambda x: float(x))  # Sorting x_axis_labels
        ...
        # Sort the DataFrame index and columns if necessary
        df = df.sort_index()  # Sorting by index (wavelength)
        df = df.reindex(sorted(df.columns, key=lambda x: float(x)), axis=1)  # Sorting columns (time or identifiers)
